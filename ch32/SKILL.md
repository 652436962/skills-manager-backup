...
| Skill | 文件 | 功能 |
|-------|------|------|
| **ch32-dev-setup** | [SKILL.md](ch32-dev-setup/SKILL.md) | 环境检测 + 工具链配置（MRS_Toolchain_Linux_X64_V* 独立包 / MRS 内置 / xPack），缺失时 AI 自动从官网下载并解压到 ~/（版本以官网 API 返回为准）；WCH-Link/OpenOCD/wchisp 支持，CLI/MRS 双路径 |
...
1. **ch32-dev-setup**：检测 Shell 环境与工具链，缺失时 AI 自动从官网（https://mounriver.com/download）下载最新版 `MRS_Toolchain_Linux_X64_V*` 并解压到指定目录（版本不硬编码，以官网 API 返回为准）；配置 WCH-Link 调试器、OpenOCD/wchisp 烧录工具
